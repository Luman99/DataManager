...
def get_ocr_qualities(ocr_engines, get_ocr_quality):
    for i, engine in enumerate(ocr_engines):
        ocr_quality = get_ocr_quality(engine)
        if i == 0:
...
def create_csv() -> List[CsvFile]:
    ...
    ocr_qualities_wer = get_ocr_qualities(ocr_engines, get_ocr_quality_cf_wer)
    ...
    for test in tests:
        list_csv = []
        tsv_file = open(f'{PATH_DATA}/test-{test}/in.tsv')
        read_tsv = csv.reader(tsv_file, delimiter="\t")
        for i, row in enumerate(read_tsv):
            if row[0] not in bad_files:
                if i % 2 == 0:
                    train_test = 'train'
                else:
                    train_test = 'test'
...
if __name__ == '__main__':
    with open(f'{PATH_DATA}/dataset_local.csv', 'w', newline='') as file:
