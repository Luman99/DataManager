...
from metrics.calculate_metrics import get_ocr_quality
from utils.Constants import PATH_DATA, PATH_BENCHMARK
from utils.CsvFile import CsvFile


def create_csv() -> List[CsvFile]:
    list_csv = []
    tsv_file = open(f'utils/in.tsv', encoding='utf-8')
    ...
    for i, (row_tsv, row_txt) in enumerate(zip(read_tsv, data_for_model)):
        ...
        csv_obj = CsvFile(name=f'{row_tsv[0]}',
                          ocr_quality_wer=round(get_ocr_quality(i, f'utils'), 2),
...
if __name__ == '__main__':
    with open(f'{PATH_BENCHMARK}/dataset.csv', 'w', newline='') as file:
        writer = csv.writer(file, delimiter=',')
        writer.writerow(['name', 'ocr_quality_wer', 'tesseract_engine_score', 'path_to_image',
                         'data_source', 'ocr_engine', 'train_test', 'language',
        ...
        for csvFile in create_csv():
            writer.writerow([csvFile.name, csvFile.ocr_quality_wer, csvFile.tesseract_engine_score,
                             ...
                             csvFile.train_test, csvFile.language, csvFile.number_of_tokens,
